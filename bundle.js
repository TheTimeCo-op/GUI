...
var post = function post(url, data, cb) {
  ...
  for (prop in data) {
    if (paramString && paramString.length > 0) paramString += "&" + prop + "=" + data[prop];else paramString = prop + "=" + data[prop];
  }
...
var put = function put(url, data, cb) {
  var xhttp = void 0;
  var paramString = void 0;
  var prop = void 0;
...
var on = function on(eventType, elementId, cb) {
  document.addEventListener(eventType, function (event) {
    var el = event.target;
    var found = void 0;
    var identifier = elementId.slice(0, 1);
...
var grabAttr = function grabAttr(attribute) {
  return event.target.attributes[attribute].nodeValue;
};

var setAttr = function setAttr(element, attributeName, attributeValue) {
  element.setAttribute(attributeName, attributeValue);
};

var grabById = function grabById(id, event) {
  if (event) {
...
var grabByClass = function grabByClass(className) {
 ...
 * @param childID 
 */
var grabByIdInContainer = function grabByIdInContainer(containerID, childID) {
  var elm = document.getElementById(containerID);
  var parent = elm ? elm.parentNode : {};
  return parent.id && parent.id === containerID ? elm : {};
};

var grabNextSibling = function grabNextSibling(className, event) {
  var sibling = void 0;
  var classList = event.target.nextElementSibling.className.split(' ');

  classList.forEach(function (siblingName) {
    if (siblingName === className) sibling = event.target.nextElementSibling;
  });
  return sibling;
};

var grabPreviousSibling = function grabPreviousSibling(className, event) {
  var sibling = void 0;
  var classList = event.target.previousElementSibling.className.split(' ');
...
var JobCardComponent = __webpack_require__(7);

var JobsComponent = function () {
  function JobsComponent(http, util, $, moment) {
  ...
  _createClass(JobsComponent, [{
    ...
    value: function getJobs() {
      ...
      this.http.get('http://localhost:9001/', function (data) {
        var response = _this.util.parse(data.response);
    ...
    value: function listen() {
      ...
      this.$.on("click", ".clock-in-btn", function (event) {
        var targetElm = event.target;
        var id = _this2.$.grabAttr('jobid');
        var time = _this2.moment().format();
        ...
        _this2.http.post(baseUrl + '/time/clockIn', params, function (data) {
          var response = _this2.util.parse(data.response);

          if (response.success) {
            var sibling = _this2.$.grabNextSibling('clock-out-btn', event);

            _this2.$.setAttr(sibling, 'timeId', response.timeId);
      ...
      this.$.on("click", ".clock-out-btn", function (event) {
        var targetElm = event.target;
        var id = _this2.$.grabAttr('timeId');
        var time = _this2.moment().format();
        ...
        _this2.http.put(baseUrl + '/time/clockOut', params, function (data) {
          var response = _this2.util.parse(data.response);

          if (response.success) {
            targetElm.style.display = 'none';
            _this2.$.grabPreviousSibling('clock-in-btn', event).style.display = 'inline-block';
          }
        });
      });
    }
...
var JobCardComponent = function () {
  function JobCardComponent(dataGroup) {
